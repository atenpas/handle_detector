...
SamplingVisualizer::createCylinder(Eigen::Vector3d pt_on_axis, Eigen::Vector3d axis_direction, double radius,
  double extent)
{
...
void
SamplingVisualizer::addCylinders(const std::vector<CylindricalShell> &shells, void* viewer_void,
  std::string handle_index, double r, double g, double b)
{
  ...
  for (int i=0; i < shells.size(); i++)
  {
    ...
    viewer->addCylinder(createCylinder(shells[i].getCentroid(), shells[i].getCurvatureAxis(),
      shells[i].getRadius(), shells[i].getExtent()), id);
    viewer->setShapeRenderingProperties(pcl::visualization::PCL_VISUALIZER_COLOR, r, g, b,
      id);
...
void
SamplingVisualizer::createViewer(PointCloud::ConstPtr cloud, std::vector<CylindricalShell> shells,
...
void
SamplingVisualizer::createViewerRGB(PointCloudRGB::ConstPtr cloud, std::vector<CylindricalShell> shells,
  Eigen::MatrixXd samples, double target_radius)
{
